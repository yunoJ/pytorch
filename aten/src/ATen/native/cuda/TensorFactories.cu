...
Tensor ARCempty_cuda(IntArrayRef size, const TensorOptions& options, c10::optional<MemoryFormat> optional_memory_format) {
  AT_ASSERT(options.backend() == at::Backend::CUDA);
  AT_ASSERT(!options.is_variable());  // is_variable should have been 'unpacked'  // TODO: remove this when Variable and Tensor are merged
  TORCH_CHECK(!options.pinned_memory(), "Only dense CPU tensors can be pinned");
  check_size_nonnegative(size);
  ...
  auto storage_impl = c10::make_intrusive<StorageImpl>(
    ...
    nelements,
//    allocator->allocate(nelements * dtype.itemsize()),
    allocator->ARCallocate(nelements * dtype.itemsize()),
    allocator,
    /*resizeable=*/true);
