...
Tensor VariableType::_adaptive_avg_pool2d(Tensor & self, IntArrayRef output_size) {
  ...
  if (compute_requires_grad( self )) {
    ...
    if (at::globalContext().ARCGlobal.isForwrad()){
      ARCCppEngine::offLoad(self, (TraceableFunction *)(grad_fn.get()), Aync, at::globalContext().ARCGlobal.getCurOid(), &(grad_fn->self));
...
static auto& registerer = globalATenDispatch()
  ...
  .registerVariableOp<Tensor & (const Tensor &, const Tensor &, const Tensor &, Scalar, Scalar)>("aten::_addr_(Tensor(a!) self, Tensor vec1, Tensor vec2, *, Scalar beta=1, Scalar alpha=1) -> Tensor(a!)", &VariableType::_addr_)
